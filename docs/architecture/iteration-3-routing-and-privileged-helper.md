...
- HTTPS 上游：`reverse_proxy https://127.0.0.1:<port>`，transport `tls.server_name = <TLSSNI>`；上游 `Host` 默认透传当前本地请求的原始 `Host`，也可选择继承 TLS SNI 或显式自定义；不写任何 `insecure_skip_verify`，默认严格校验。
...
6. UI：Route 管理（域名、hosts/Caddy 开关、HTTPS SNI、上游 `Host` 三种模式）+ 非本地域名确认对话框 + RouteStatus 展示。
